...
abstract class ParameterHandler<T> {
    ...
    static final class ModelMap<T> extends ParameterHandler<T> {

        private final boolean encoded;
        private final boolean excludeNull;
        private final boolean excludeBlank;

        ModelMap(boolean encoded,
                boolean excludeNull, boolean excludeBlank) {
            this.encoded = encoded;
        ...
        void apply(RequestBuilder builder, @Nullable T value) throws IOException {
            ...
            Map<String, Object> fieldMap = JsonTool.convertValue(value, Map.class);
            if (fieldMap == null) {
                return; // Skip null converted values
            }
            for (Map.Entry<String, Object> entry : fieldMap.entrySet()) {
                ...
                if ((entryValue == null) && excludeNull) {
                    continue;
                }
                if (excludeBlank && (entryValue instanceof String) && StringTool
                        .isBlank((String) entryValue)) {
                    continue;
                }
                String fieldEntry = StringTool.valueOf(entryValue);
                builder.addFormField(entryKey, fieldEntry, encoded);
            }
        }
    }
